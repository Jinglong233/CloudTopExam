...
@Component
@Scope(value = "session")
public class XunFei extends WebSocketListener {
    ...
    // 构造函数
    public XunFei(String userId, Boolean wsCloseFlag) {
        this.userId = userId;
    ...
    public void sendAnalizes() throws Exception {
        ...
        if (totalFlag) {
            ...
            for (int i = 0; i < 1; i++) {
                totalAnswer = "";
                WebSocket webSocket = client.newWebSocket(request, new XunFei(i + "",
                        false));
            }
        } else {
            Thread.sleep(200);
